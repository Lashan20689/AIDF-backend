...
async function fulfillCheckout(sessionId: string) {
    ...
    if (booking.paymentStatus !== "PENDING") throw new Error("Payment is not pending");
...
export const createCheckoutSession = async (req: Request, res: Response) => {
    try {
        ...
        if (!hotel.stripePriceId) throw new Error("Stripe price ID is missing");

        const session = await stripe.checkout.sessions.create({
            ui_mode: "embedded",
            line_items: [{
                price: hotel.stripePriceId as string,
